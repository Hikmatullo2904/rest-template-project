...
public class AuthService {
    ...
    @Value("${server.external.url.auth.authenticate}")
    private String authenticateUrl;

    @Value("${server.external.url.auth.refresh}")
    private String refreshTokenUrl;

    public String getToken(String username) {
        Optional<ExternalApiToken> token = tokenRepository.findExternalApiTokenByRefreshToken(username);
        if(token.isEmpty()) {
